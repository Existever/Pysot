...
class SiamRPNLTTracker(SiamRPNTracker):
    ...
    def track(self, img):
        ...
        """
        w_z = self.size[0] + cfg.TRACK.CONTEXT_AMOUNT * np.sum(self.size)           #带上下文的区域为模板区域
        h_z = self.size[1] + cfg.TRACK.CONTEXT_AMOUNT * np.sum(self.size)
        s_z = np.sqrt(w_z * h_z)                                             #对应图像原始分辨的模板尺寸
        ...
        score_size = (instance_size - cfg.TRACK.EXEMPLAR_SIZE) // \
            cfg.ANCHOR.STRIDE + 1 + cfg.TRACK.BASE_SIZE             #输出相关图是通过卷积实现的，(w-k)/s+1
        hanning = np.hanning(score_size)                            #生成hanning窗口
        window = np.outer(hanning, hanning)
        window = np.tile(window.flatten(), self.anchor_num)
        anchors = self.generate_anchor(score_size)

        s_x = s_z * (instance_size / cfg.TRACK.EXEMPLAR_SIZE)        #对应图像原始分辨的搜索区域尺寸
