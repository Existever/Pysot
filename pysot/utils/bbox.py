...
def get_axis_aligned_bbox(region):
    ...
    if nv == 8:   #[x1,y1,x2,y2,x3,y3,x4,y4]
        cx = np.mean(region[0::2])      #间隔两个取，偶数位置放置x坐标，取均值得到中心cx
        cy = np.mean(region[1::2])      #间隔两个取，计数位置放置y坐标，取均值得到中心cy
        x1 = min(region[0::2])
        x2 = max(region[0::2])
        y1 = min(region[1::2])
        y2 = max(region[1::2])
        A1 = np.linalg.norm(region[0:2] - region[2:4]) * \
            np.linalg.norm(region[2:4] - region[4:6])       #计算斜框的面积
        A2 = (x2 - x1) * (y2 - y1)                         #计算正立起的外接矩形的面积
        s = np.sqrt(A1 / A2)
        w = s * (x2 - x1) + 1                   #w,h通过斜框面积和正立框的面积的比值的平方根，稍微调小一些，否则包含了比较多的背景
        h = s * (y2 - y1) + 1
    else:
        x = region[0]
        y = region[1]
